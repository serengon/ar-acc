...
RETURN node, score, node_labels,
       ...
       coalesce(node.cuil, node.cuit, node.contract_id, node.sanction_id, node.amendment_id, node.cnes_code, node.finance_id, node.embargo_id, node.school_id, node.convenio_id, node.stats_id, elementId(node)) AS document_id
ORDER BY score DESC
SKIP $skip
LIMIT $limit
