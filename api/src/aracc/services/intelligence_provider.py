...
from aracc.models.entity import ExposureFactor, ExposureResponse, SourceAttribution
from aracc.models.pattern import PATTERN_METADATA, PatternResult
from aracc.services.neo4j_service import execute_query, execute_query_single
...
def _load_pattern_queries() -> Mapping[str, str]:
    module = import_module("aracc.services.pattern_service")
...
def _load_compute_exposure() -> _ComputeExposure:
    module = import_module("aracc.services.score_service")
    module_any = cast("Any", module)
...
def _full_modules_available() -> bool:
    return (
        find_spec("aracc.services.pattern_service") is not None
        and find_spec("aracc.services.score_service") is not None
    )
