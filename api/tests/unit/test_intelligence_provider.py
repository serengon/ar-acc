...
from aracc.services import intelligence_provider as provider_module
...
def test_community_patterns_have_query_files() -> None:
    from aracc.services.neo4j_service import CypherLoader
...
async def test_community_provider_enforces_public_evidence_fields() -> None:
    ...
    with (
        patch(
            "aracc.services.intelligence_provider.execute_query_single",
        ...
        patch(
            "aracc.services.intelligence_provider.execute_query",
            new_callable=AsyncMock,
            return_value=[
                {
