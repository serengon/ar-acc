...
from aracc.models.pattern import PatternResult
...
async def test_search_hides_person_nodes_in_public_mode(
    ...
    with patch(
        "aracc.routers.search.execute_query",
...
async def test_public_patterns_company_endpoint_when_enabled(client: AsyncClient) -> None:
    with (
        patch("aracc.routers.public.settings.patterns_enabled", True),
        patch(
            "aracc.routers.public.execute_query_single",
            new_callable=AsyncMock,
            return_value={
        ...
        patch(
            "aracc.routers.public._PUBLIC_PROVIDER.run_pattern",
            new_callable=AsyncMock,
            return_value=[
                PatternResult(
...
async def test_public_graph_company_filters_person_nodes(client: AsyncClient) -> None:
    with (
        ...
        patch(
            "aracc.routers.public.execute_query",
            new_callable=AsyncMock,
            return_value=[
                {
                    "nodes": [
...
async def test_stats_hides_person_count_in_public_mode(
    ...
    import aracc.routers.meta as meta_mod
    ...
    with patch(
        "aracc.routers.meta.execute_query_single",
        new_callable=AsyncMock,
        return_value=fake_record,
    ), patch(
        "aracc.routers.meta.load_source_registry",
        return_value=[],
    ), patch(
        "aracc.routers.meta.source_registry_summary",
        return_value={
...
async def test_timeline_sanitizes_properties_in_public_mode(
    ...
    with patch(
        "aracc.routers.entity.execute_query",
        new_callable=AsyncMock,
