...
if [[ "$PROFILE" == "full" ]]; then
  ...
  if [[ "$DOWNLOAD" == "true" ]]; then
    ...
    if [[ "$PIPELINES" == *"cnpj"* ]]; then
      (cd etl && uv run aracc-etl download --output-dir ../data/cnpj --files "$CNPJ_FILES" --skip-existing)
    fi
  ...
  for source in "${pipeline_array[@]}"; do
    ...
    (cd etl && uv run aracc-etl run --source "$source" --neo4j-uri "$NEO4J_URI" --neo4j-password "$NEO4J_PASSWORD" --neo4j-database "$NEO4J_DATABASE" --data-dir ../data)
  done
fi
